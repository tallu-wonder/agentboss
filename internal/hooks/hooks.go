// Package hooks installs agentdeck's status hooks into Claude Code's
// settings.json. The hooks call `agentdeck hook`, a fast subcommand that
...
// are only appended if no "agentdeck hook" command is already registered for
// that event.
package hooks
...
// Events agentdeck listens to. Together they yield the full status lifecycle:
...
// ourCommand matches ONLY a command agentdeck itself would have written: an
// optionally quoted path whose final element is the agentdeck binary, followed
// by the bare `hook` subcommand.
//
// The match has to be this strict because a match is rewritten in place. A
// loose "mentions agentdeck and hook" test would also match a user's own
// wrapper — `notify.sh && agentdeck hook`, say — and replacing that would
// destroy configuration agentdeck did not write.
var ourCommand = regexp.MustCompile(`^'?(?:[^\s'"|;&]*/)?agentdeck'? hook$`)

// isOurs recognizes a previously installed agentdeck hook command, with or
...
// Install ensures every agentdeck hook is present in settingsPath. binPath is
// the absolute path to the agentdeck binary. Returns true if the file was
// modified.
func Install(settingsPath, binPath string) (bool, error) {
	...
	// Keep one copy of the file as it was before agentdeck first touched it.
	...
	if len(data) > 0 {
		backup := settingsPath + ".agentdeck-backup"
...
const (
	upsertMissing upsertResult = iota // no agentdeck entry: append one
	upsertCurrent                     // entry present and up to date
	upsertHealed                      // entry present, stale path rewritten
)

// upsertOurs finds an existing agentdeck hook entry; when its command is
